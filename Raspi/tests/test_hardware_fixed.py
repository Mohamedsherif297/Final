#!/usr/bin/env python3
"""
Hardware Component Test Script - Fixed Version
...
Usage:
    python3 test_hardware_fixed.py
...
# Add Raspi/Drivers to Python path to fix import issues
...
try:
    ...
    print(f"Raspi path: {raspi_path}")
    print(f"Drivers path: {drivers_path}")
    sys.exit(1)
...
class HardwareTest:
    ...
    def test_servos(self):
        """Test servo movements - PAN ONLY"""
        self.print_header("TEST 2: SERVO CONTROLLER (PAN ONLY)")
        
        if not self.running:
            return
        
        # Center position
        self.print_test("Pan Servo", "Moving to CENTER position (0°)")
        try:
            self.hardware_manager.servo.set_angle(pan=0, tilt=0)
        ...
        # Pan left
        self.print_test("Pan Servo", "Panning LEFT (-90°)")
        try:
        ...
        # Pan right
        self.print_test("Pan Servo", "Panning RIGHT (+90°)")
        try:
            self.hardware_manager.servo.set_angle(pan=90, tilt=0)
        ...
        # Pan to 45 degrees
        self.print_test("Pan Servo", "Panning to +45°")
        try:
            self.hardware_manager.servo.set_angle(pan=45, tilt=0)
            self.wait_with_countdown(self.test_duration, "Pan +45°")
            print("✅ Pan +45° complete")
        except Exception as e:
            print(f"❌ Pan +45° test failed: {e}")
        ...
        # Return to center
        self.print_test("Pan Servo", "Returning to CENTER")
        try:
            self.hardware_manager.servo.set_angle(pan=0, tilt=0)
            time.sleep(1)
            print("✅ Returned to center")
        except Exception as e:
            print(f"❌ Final center failed: {e}")
        
        print("\n✅ Pan servo tests complete")
        print("ℹ️  Note: Tilt servo test skipped (only pan servo configured)")
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            print("   ✅ Pan Servo (left, right, center)")
            ...
            print("\n💡 Note: Only pan servo tested (tilt servo skipped)")
